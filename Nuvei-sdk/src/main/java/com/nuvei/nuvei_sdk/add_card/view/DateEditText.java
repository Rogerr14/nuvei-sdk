...
import com.nuvei.nuvei_sdk.add_card.helper.DateUtils;
import java.util.Calendar;

public class DateEditText  extends EditTextWidget {

    static final int INVALID_INPUT = -1 ;
    private static final  int MAX_INPUT_LENGTH = 5;
    private IExpireDateListener expireDateListener;
    ...
    public void setExpiryDateEditListener(IExpireDateListener listener) {
        expireDateListener = listener;
    }
        ...
        addTextChangedListener(new TextWatcher() {
            ...
            public void afterTextChanged(Editable s) {
                ...
                if (parts[0].length() == 2 && parts[1].length() == 2) {
                    boolean wasComplete = isDateValid;
                    updateInputValues(parts);
                    // Here, we have a complete date, so if we've made an invalid one, we want
                    // to show an error.
                    shouldShowError = !isDateValid;
                    if (!wasComplete && isDateValid && expireDateListener != null) {
                        expireDateListener.onExpiryDateComplete();
                    }
                } else {
                    isDateValid = false;
                }
